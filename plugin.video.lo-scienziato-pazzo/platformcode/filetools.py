...
from platformcode import scrapertools
    ...
    try:
        if xbmc_vfs and vfs:
            ...
            if os.path.exists(new_path):
                logger.info("Essiste %s", new_path)
                os.remove(new_path)
                logger.info("Essistia %s", new_path)
            os.rename(path, new_path)
    except:
        logger.exception("ERROR when renaming the file: %s to %s, vfs %s"%(path, new_name, vfs))
        if not silent:
            logger.error(traceback.format_exc())
